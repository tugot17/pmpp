...
![alt text](exercise_2a.jpeg)

![alt text](exercise_2b.jpeg)
...
There are `1000 x 512 = 512,000` threads in the gird. The variable will be created and stored for every single one of them, meaning it will be created `512,000` times. 
...
It will work correctly only for `TILE_SIZE=1`, as it is a trivial case where the kernel doesn't actually do any work whatsoever. It works with only a single thred in a block, meaning there is no need to syncsynchronize anything. 
...
To estimate the amount of shared memory used per block, we need to identify all `__shared__` variables in the kernel and multiply it by their corresponding data type size, accounting for the array dimensions. In case of the `foo` kernel lines 4 and 5, `float y_s` and `float b_s[128]`. `y_s` is a float, so it is `4 bytes` per block; `b_s` is an array of `128` floats, so `128 x 4 = 512`, `4 + 512 = 516` bytes of shared memory per block in total. 
...
So the ratio is 7 operations per 5 loads. Since each of the loaded variables is a float—4 bytes of memory. We have 7 operations and `5x4bytes = 20bytes` of memory loaded, so `7 operations/20 Bytes` or `0.7 operation/byte`.
