...
class QuadtreeVisualizer:
    ...
    def generate_sample_points(self, num_points=1000, pattern="random"):
        ...
        if pattern == "random":
            ...
            x = (xx.flatten() + np.random.normal(0, 0.2, xx.size))[:num_points].astype(np.float32)
            y = (yy.flatten() + np.random.normal(0, 0.2, yy.size))[:num_points].astype(np.float32)
    ...
    def visualize_quadtree_division(self, x_orig, y_orig, x_reordered, y_reordered, 
                                  max_depth=4, min_points_per_node=10):
        ...
        ax2.scatter(x_reordered, y_reordered, alpha=0.7, s=20, 
                   c=range(len(x_reordered)), cmap='viridis')
        ax2.set_xlim(0, 10)
        ax2.set_ylim(0, 10)
        ...
        self._draw_quadtree_boundaries(ax3, x_orig, y_orig, 0, 0, 10, 10, 
                                     max_depth, min_points_per_node, 0)
        ax3.set_xlim(0, 10)
        ax3.set_ylim(0, 10)
    ...
    def _draw_quadtree_boundaries(self, ax, x, y, min_x, min_y, max_x, max_y, 
                                max_depth, min_points_per_node, current_depth):
        ...
        # Draw center lines
        ax.axvline(x=center_x, ymin=(min_y/10), ymax=(max_y/10), 
                  color='red', alpha=0.5, linewidth=0.8)
        ax.axhline(y=center_y, xmin=(min_x/10), xmax=(max_x/10), 
                  color='red', alpha=0.5, linewidth=0.8)
        
        # Recursively divide into 4 quadrants
        # Top-left
        self._draw_quadtree_boundaries(ax, x, y, min_x, center_y, center_x, max_y,
                                     max_depth, min_points_per_node, current_depth + 1)
        # Top-right  
        self._draw_quadtree_boundaries(ax, x, y, center_x, center_y, max_x, max_y,
                                     max_depth, min_points_per_node, current_depth + 1)
        # Bottom-left
        self._draw_quadtree_boundaries(ax, x, y, min_x, min_y, center_x, center_y,
                                     max_depth, min_points_per_node, current_depth + 1)
        # Bottom-right
        self._draw_quadtree_boundaries(ax, x, y, center_x, min_y, max_x, center_y,
                                     max_depth, min_points_per_node, current_depth + 1)
...
def main():
    ...
    for pattern in patterns:
        ...
        visualizer.visualize_quadtree_division(
            x_orig, y_orig, x_reordered, y_reordered,
            max_depth, min_points_per_node
        )
        
        # Show statistics
        print(f"  Original points range: X[{x_orig.min():.2f}, {x_orig.max():.2f}], "
