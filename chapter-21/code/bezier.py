...
def simple_tessellate_dynamic(control_points_list):
    ...
    lib.tessellate_bezier_curves_dynamic.argtypes = [ctypes.POINTER(BezierLineC), ctypes.c_int]
...
def plot_curves_comparison(control_points_list, tessellated_static, tessellated_dynamic):
    ...
    for i, (cp, tess_points, color) in enumerate(zip(control_points_list, tessellated_static, colors)):
    ...
    for i, (cp, tess_points, color) in enumerate(zip(control_points_list, tessellated_dynamic, colors)):
        if len(tess_points) > 0:
            ax3.plot(tess_points[:, 0], tess_points[:, 1], 'o-', color=color, 
                    markersize=4, linewidth=2, alpha=0.8, 
                    label=f'Curve {i+1} ({len(tess_points)} pts)')
            
            ax3.plot(tess_points[0, 0], tess_points[0, 1], 's', color=color, 
                    markersize=8, markeredgecolor='black', markeredgewidth=1)
            ax3.plot(tess_points[-1, 0], tess_points[-1, 1], '^', color=color, 
                    markersize=8, markeredgecolor='black', markeredgewidth=1)
...
def analyze_tessellation_comparison(control_points_list, tessellated_static, tessellated_dynamic):
    """Analyze and compare static vs dynamic results"""
    print("\n" + "="*80)
    print("TESSELLATION COMPARISON: STATIC vs DYNAMIC PARALLELISM")
    print("="*80)
    
    for i, (cp, tess_static, tess_dynamic) in enumerate(zip(control_points_list, tessellated_static, tessellated_dynamic)):
        if len(tess_static) > 0 and len(tess_dynamic) > 0:
            ...
            if len(tess_static) == len(tess_dynamic):
                max_diff = np.max(np.abs(tess_static - tess_dynamic)) if len(tess_static) > 0 else 0
                print(f"  Max Difference: {max_diff:.8f}" + (" (Identical)" if max_diff < 1e-6 else " (Different)"))
            else:
                print(f"  Result: Different number of points!")
            print()
...
def benchmark_performance(curves):
    ...
    for size in test_sizes:
        ...
        static_time = triton.testing.do_bench(
            lambda: simple_tessellate_static(test_curves),
            warmup=5,
            rep=25
        )
        ...
        dynamic_time = triton.testing.do_bench(
            lambda: simple_tessellate_dynamic(test_curves),
            warmup=5,
            rep=25
        )
        ...
        print(f"  Speedup: {speedup:.2f}x {'(Dynamic faster)' if speedup > 1 else '(Static faster)'}")
